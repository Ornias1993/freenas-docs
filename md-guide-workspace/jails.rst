...
from the host FreeNAS\ :sup:`®` system. FreeNAS\ :sup:`®` uses
...
and configurations within a jail are isolated from both the FreeNAS\ :sup:`®`
...
pools exist on the FreeNAS\ :sup:`®` system.
...
FreeNAS\ :sup:`®` has two options to create a jail. The :guilabel:`Jail Wizard`
...
:menuselection:`Jails -->` :guilabel:`ADD`.
...
FreeNAS\ :sup:`®` system. Newer releases are not shown.
...
- The jail can use
  ...
  which uses the FreeNAS\ :sup:`®` IP address and sets a unique port for the jail to use.
...
- Networking can be manually configured by entering values for the
  ...
  manually configuring IP addresses, FreeNAS\ :sup:`®` automatically assigns the given IP
...
Setting a proxy in the FreeNAS\ :sup:`®`
...
.. table:: Basic Properties
   ...
   +---------------------------+-------------------+---------------------------------------------------------------------------------------------------------+
   | Release                   | drop-down menu    | Required. Jails can run FreeBSD versions up to the same version as the host FreeNAS\ :sup:`®` system.   |
...
.. warning:: Modify the IP address information for a jail by clicking
   ...
   ensures the changes are saved and will survive a jail or FreeNAS\ :sup:`®`
   reboot.
...
.. tip:: A root shell can also be opened for a jail using the FreeNAS\ :sup:`®` UI
...
is configured on the FreeNAS\ :sup:`®` system. It is possible to give a FreeBSD
jail access to an area of storage on the FreeNAS\ :sup:`®` system. This is useful
for applications or plugins that store large amounts of data or if an
application in a jail needs access to data stored on the FreeNAS\ :sup:`®` system.
For example, Transmission is a plugin that stores data using BitTorrent.
The FreeNAS\ :sup:`®` external storage is added using the
...
* :guilabel:`Source`: is the directory or dataset on the FreeNAS\ :sup:`®` system
  which will be accessed by the jail. FreeNAS\ :sup:`®` creates the directory
...
* :guilabel:`Destination`: Browse to an existing and **empty** directory
  ...
  and FreeNAS\ :sup:`®` automatically creates a new directory. New directories
...
access data stored on the FreeNAS\ :sup:`®` system. Before selecting the
...
from the users and groups of the FreeNAS\ :sup:`®` system.
...
#.  On the FreeNAS\ :sup:`®` system, create a user account and group account
...
:guilabel:`Source` as it contains the files stored on the FreeNAS\ :sup:`®`
...
.. warning:: Remember that added storage is
   just a pointer to the selected storage directory on the FreeNAS\ :sup:`®`
   ...
   **directory in the jail are really deleted from the**
   :guilabel:`Source`
   **directory on the** FreeNAS\ :sup:`®` **system.**
...
* It takes time. Depending upon the size of the application, the
  ...
  available, and the current load on the FreeNAS\ :sup:`®` system, the time
...
.. note:: If the port does not provide any compile options, it saves
   time and preserves the FreeNAS\ :sup:`®` system resources to use the
