...
Go to :menuselection:`Tasks --> Cron Jobs` and click :guilabel:`ADD` to
create a cron job.
...
FreeNAS\ :sup:`®` provides the ability to schedule commands or scripts to run
...
.. table:: Init/Shutdown Command or Script Options
   ...
   |             |                | * *Post Init*: at the end of the boot process, before FreeNAS\ :sup:`®` services start       |
...
FreeNAS\ :sup:`®` can be configured as either an *rsync client* or an
*rsync server*. The opposite end of the connection can be another
FreeNAS\ :sup:`®` system or any other system running rsync. In FreeNAS\ :sup:`®` terminology,
an *rsync task* defines which data is synchronized between the two
systems. To synchronize data between two FreeNAS\ :sup:`®` systems, create the
*rsync task* on the *rsync client*.

FreeNAS\ :sup:`®` supports two modes of rsync operation:
...
* **Module:** exports a directory tree, and the configured
  ...
  server. It can be defined in the FreeNAS\ :sup:`®` web interface under
...
provides a configuration example between two FreeNAS\ :sup:`®` systems for each
...
.. table:: Rsync Configuration Options
   ...
   | Setting                      | Value          | Description                                                                               |
   +==============================+================+===========================================================================================+
   | Path                         | browse button  | :guilabel:`Browse` to the path to be copied. FreeNAS\ :sup:`®` verifies that the          |
   |                              |                | remote path exists. :ref:`FreeBSD path length limits <Path and Name Lengths>`             |
   |                              |                | apply on the FreeNAS\ :sup:`®` system. Other operating systems can have                   |
...
the two following FreeNAS\ :sup:`®` systems:
...
:menuselection:`Tasks --> Rsync Tasks`, :guilabel:`ADD`.
In this example:
...
FreeNAS\ :sup:`®` supports RSA keys for SSH. When creating the key, use
...
and :guilabel:`ADD` to add a new scheduled S.M.A.R.T. test.
...
and click :guilabel:`ADD`. This opens the screen shown in
...
.. table:: Periodic Snapshot Options
   ...
   |                    |                            | to be compatible with snapshot schedules or replications created in FreeNAS\ :sup:`®`                            |
...
Examples in this section refer to the FreeNAS\ :sup:`®` system with the original
datasets for snapshot and replication as *Primary* and the FreeNAS\ :sup:`®`
system that is storing replicated snapshots as *Secondary*.
...
snapshots of the chosen datasets. When no snapshots exist, FreeNAS\ :sup:`®`
...
FreeNAS\ :sup:`®` will snapshot the source datasets and immediately replicate
...
legacy engine replications from FreeNAS\ :sup:`®` 11.1 or earlier, or even
creating a one-time replication that is not linked to a periodic
snapshot task.

Go to
:menuselection:`System --> Replication Tasks`,
click :guilabel:`ADD` and :guilabel:`ADVANCED REPLICATION CREATION` to see
...
.. table:: Replication Task Options
   ...
   |                           |           |                |   transfer speeds. By default, this is supported by FreeNAS\ :sup:`®` systems with 11.2 or later installed      |
   |                           |           |                |   (11.3 or later is recommended). Destination systems that do not have FreeNAS\ :sup:`®` 11.2 or later          |
   ...
   |                           |           |                | * *LEGACY* uses the legacy replication engine from FreeNAS\ :sup:`®` 11.2 and earlier.                          |
   |                           |           |                |                                                                                                                 |
   +---------------------------+-----------+----------------+-----------------------------------------------------------------------------------------------------------------+
   ...
   | 500M, 2 TB)               |           |                | This is a :ref:`humanized field <Humanized Fields>`.                                                                                               |
   +---------------------------+-----------+----------------+-----------------------------------------------------------------------------------------------------------------+
...
both computers are running FreeNAS\ :sup:`®`, but other operating systems might
...
:file:`/var/log/debug.log` on the FreeNAS\ :sup:`®` system for errors.
...
alerts of impending disk failures. FreeNAS\ :sup:`®` makes it easy to schedule
...
Click :guilabel:`ADD` to display the :guilabel:`Add Cloud Sync` menu shown in
...
This example shows a *Push* cloud sync that copies files from a FreeNAS\ :sup:`®`
pool to a cloud service provider.

The cloud service provider was configured with a location to store data
received from the FreeNAS\ :sup:`®` system.

In the FreeNAS\ :sup:`®` web interface, go to
:menuselection:`System --> Cloud Credentials`
and click :guilabel:`ADD` to configure the cloud service provider credentials:
...
and click :guilabel:`ADD` to create a cloud sync job. The
