...
Services that ship with FreeNAS\ :sup:`®` are configured, started, or stopped
in :guilabel:`Services`.
FreeNAS\ :sup:`®` includes these built-in services:
...
This section demonstrates starting a FreeNAS\ :sup:`®` service and the available
configuration options for each FreeNAS\ :sup:`®` service.
...
:guilabel:`ADD`. In contrast, global settings which apply to all AFP shares
are configured in
...
Dynamic DNS (DDNS) is useful if the FreeNAS\ :sup:`®` system is connected to an
...
a domain name, allowing access to the FreeNAS\ :sup:`®` system even if the IP
...
FreeNAS\ :sup:`®` uses the `proftpd <http://www.proftpd.org/>`__ FTP server to
...
FreeNAS\ :sup:`®` system. The disadvantage of FTP is that it is considered to
...
FreeNAS\ :sup:`®` system is not accessible from the Internet and everyone in
...
passwords on the FreeNAS\ :sup:`®` system.
...
#.  Give the built-in ftp user account permissions to the
    ...
    * :guilabel:`Mode`: review that the permissions are appropriate
    ...
    .. note:: For FTP, the type of client does not matter when it
       ...
       ACLs are used even if Windows clients are accessing FreeNAS\ :sup:`®` via
       FTP.
...
* IP address of the FreeNAS\ :sup:`®` server: *192.168.1.113*
...
local site (their system) and the remote site (the FreeNAS\ :sup:`®` system).
...
the FreeNAS\ :sup:`®` system, either create a user account for each user or import
...
#.  When :ref:`Active Directory` or :ref:`LDAP` are not being used,
    ...
    :menuselection:`Accounts --> Users`, and clicking :guilabel:`ADD`.
...
#.  Set the permissions for each dataset by navigating to
    ...
    .. note:: For FTP, the type of client does not matter when it
       ...
       used, even if Windows clients will be accessing FreeNAS\ :sup:`®` via
       FTP.
...
FreeNAS\ :sup:`®` system, the *Username* of a user that is associated with
...
the remote site (their dataset on the FreeNAS\ :sup:`®` system).
...
#.  Specify secure FTP when accessing the FreeNAS\ :sup:`®` system. For
    ...
    will be presented with the certificate of the FreeNAS\ :sup:`®` system.
...
running. To see the error message that occurs when FreeNAS\ :sup:`®` tries to
...
server with the FreeNAS\ :sup:`®` system hostname and IP address, or add an entry
for the IP address of the FreeNAS\ :sup:`®` system in the
...
to
:menuselection:`Sharing --> Unix (NFS) Shares` and clicking :guilabel:`ADD`.
...
:guilabel:`Rsync Module` tab, and click :guilabel:`ADD`.
...
Amazon S3 cloud storage. The FreeNAS\ :sup:`®` S3 service uses
`Minio <https://minio.io/>`__
to provide S3 storage hosted on the FreeNAS\ :sup:`®` system itself. Minio also
...
.. table:: S3 Configuration Options
   ...
   |                 |                |                                                                                                                                 |
   +-----------------+----------------+---------------------------------------------------------------------------------------------------------------------------------+
   | Enable Browser  | checkbox       | Set to enable the web user interface for the S3 service. Access the minio web interface by entering the IP address and port number   |
...
.. note:: After starting the SMB service, it can take several minutes
   ...
   to occur and for the FreeNAS\ :sup:`®` system to become available in
   Windows Explorer.
...
provides a seamless monitoring service through SNMP for FreeNAS\ :sup:`®` called
...
encrypted network. When a FreeNAS\ :sup:`®` system is used as an SSH
...
This section shows the FreeNAS\ :sup:`®` SSH configuration options,
...
.. table:: SSH Configuration Options
   ...
   |                               |                |          |                                                                                                     |
   +-------------------------------+----------------+----------+-----------------------------------------------------------------------------------------------------+
   | Allow kerberos authentication | checkbox       | ✓        | Ensure :ref:`Kerberos Realms` and :ref:`Kerberos Keytabs` are configured and FreeNAS\ :sup:`®` can  |
...
can use :command:`ssh` to log into the FreeNAS\ :sup:`®` system over the network.
User accounts are created by navigating to
:menuselection:`Accounts --> Users`, and clicking :guilabel:`ADD`.
The user home directory is the pool or dataset specified in the
:guilabel:`Home Directory` field of the FreeNAS\ :sup:`®` account for that user.
...
on the FreeNAS\ :sup:`®` system, while restricting them from logging into the
...
If the FreeNAS\ :sup:`®` system will be used to store images and configuration
...
FreeNAS\ :sup:`®` uses `NUT <https://networkupstools.org/>`__ (Network UPS Tools)
to provide UPS support. If the FreeNAS\ :sup:`®` system is connected to a UPS
...
.. table:: UPS Configuration Options
   ...
   |                               |                | *fixmepass*.                                                                                                           |
   +-------------------------------+----------------+------------------------------------------------------------------------------------------------------------------------+
   | Send Email Status Updates     | checkbox       | Set to enables the FreeNAS\ :sup:`®` system to send email updates to the configured :guilabel:`Email` field.           |
   |                               |                |                                                                                                                        |
   +-------------------------------+----------------+------------------------------------------------------------------------------------------------------------------------+
   ...
   |                               |                |                                                                                                                        |
   +-------------------------------+----------------+------------------------------------------------------------------------------------------------------------------------+
   | Power Off UPS                 | checkbox       | Set for the UPS to power off after shutting down the FreeNAS\ :sup:`®` system.                                         |
   |                               |                |                                                                                                                        |
   +-------------------------------+----------------+------------------------------------------------------------------------------------------------------------------------+
...
This can show in FreeNAS\ :sup:`®` logs as a recurring error like:
...
:menuselection:`Sharing --> WebDAV Shares`, and clicking :guilabel:`ADD`.
