...
configuration options within the FreeNAS\ :sup:`®` web interface. After successfully
...
.. table:: Active Directory Configuration Options
   ...
   +--------------------------+---------------+----------+-------------------------------------------------------------------------------------------------------------------------------+
   | Disable FreeNAS Cache    | checkbox      | ✓        | Disable caching AD users and groups. Setting this hides all AD users and groups from web interface drop-down menus and             |
...
The Active Directory users and groups that are imported to the FreeNAS\ :sup:`®`
...
report at `<https://bugs.ixsystems.com>`. Include the commands in the order in which
...
a Key Distribution Center (KDC), click :guilabel:`ADD` to add the realm. The
...
After the keytab is generated, add it to the FreeNAS\ :sup:`®` system using
