...
system with one disk. FreeNAS\ :sup:`®` automatically schedules a monthly scrub
...
FreeNAS\ :sup:`®`, :ref:`Pools` is used to create or extend pools.
...
accessing the compressed data. ZFS pools created on FreeNAS\ :sup:`®` version
...
upgrade**. In FreeNAS\ :sup:`®`, a snapshot of the dataset the operating system
...
read latency. FreeNAS\ :sup:`®` adds ARC stats to
...
* Some older ZFS documentation recommends that a certain number of
  ...
  for FreeNAS\ :sup:`®` 9.2.1 and higher, this is no longer true. See
...
ZFS pool are supported by both platforms. FreeNAS\ :sup:`®` uses OpenZFS and each
new version of FreeNAS\ :sup:`®` keeps up-to-date with the latest feature flags
