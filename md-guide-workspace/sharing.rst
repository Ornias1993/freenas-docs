...
:menuselection:`Sharing --> Apple (AFP)`, then :guilabel:`ADD`.
...
and click :guilabel:`ADD` to display the screen shown in
...
and click :guilabel:`ADD` as shown in
...
and clicking :guilabel:`ADD`. The screen is shown in
...
and click :guilabel:`ADD`. In the example shown in
...
.. note:: The web interface does not allow reducing the size of the
...
and clicking :guilabel:`ADD`. :numref:`Figure %s <nfs_share_wiz_fig>` shows
...
* **share_name:** is configured by clicking
  :menuselection:`Sharing --> WebDAV Shares`, then :guilabel:`ADD`.
...
:menuselection:`Sharing --> WebDAV Shares` and click :guilabel:`ADD`,
...
and click :guilabel:`ADD`. Use a descriptive name for the group like
...
* Users cannot delete shadow copies on the Windows system due to the
  ...
  from the FreeNAS\ :sup:`®` web interface. The only way to disable shadow
