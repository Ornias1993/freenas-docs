...
Several command line utilities which are provided with FreeNAS\ :sup:`®` are
...
* :ref:`freenas-debug`: the backend used to dump FreeNAS\ :sup:`®` debugging
  information
...
FreeNAS\ :sup:`®` includes the iperf server. To perform network testing,
install an iperf client on a desktop system that has
network access to the FreeNAS\ :sup:`®` system. This section demonstrates
how to use the
...
Start the iperf server on FreeNAS\ :sup:`®` when the client is ready.

.. note:: Beginning with FreeNAS\ :sup:`®` version 11.1, both `iperf2
...
FreeNAS\ :sup:`®` system, specify the running time for the test under
...
running on the FreeNAS\ :sup:`®` system. This helps to determine if the traffic
...
FreeNAS\ :sup:`®` ships with IOzone so it can be run from Shell.
When using IOzone on FreeNAS\ :sup:`®`, :command:`cd` to a directory in a
...
Starting with version 9.2.1, FreeNAS\ :sup:`®` enables compression on newly
...
FreeNAS\ :sup:`®` provides two command line scripts which can be manually run
from :ref:`Shell`:
...
FreeNAS\ :sup:`®` includes the :command:`tw_cli` command line utility for
...
them available in the FreeNAS\ :sup:`®` web interface.
...
:MegaRAID SAS family of RAID controllers. FreeNAS\ :sup:`®` also includes the
...
The FreeNAS\ :sup:`®` web interface provides an option to save debugging information to a
